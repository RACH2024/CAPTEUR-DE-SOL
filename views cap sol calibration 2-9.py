        ...
        if event == 'up' :
            ...
            try :
                ...
                if data['deviceInfo']['devEui'] == 'a84041d10858e027':  # Vérifie si c'est bien le capteur de sol
                    print("📡 Données reçues du Capteur de sol (calibré par gravimétrie)")

                    object_data = data.get('object', {})
                    batterie = object_data.get('Batterie', '0')
                    ...
                    def calib_capteur7(x):
                        return 0.489*x + 8.3

                    def calib_capteur8(x):
                        return 0.517*x + 7.96

                    def calib_capteur9(x):
                        return 0.517*x + 7.96
                    ...
                    for key in object_data.keys():
                        if not key.startswith("Capteur_"):
                            continue
                        index_txt = key.split("_", 1)[1]
                        if index_txt.isdigit():
                            capteur_indexes.append(int(index_txt))

                    capteur_indexes = sorted(set(capteur_indexes))
                    print("🧾 Index capteurs détectés dans la trame :", capteur_indexes)

                    z = 0
                    for i in capteur_indexes:
                        if i < 1 or i > 9:
                            continue

                        capteur_key = f"Capteur_{i}"
                        capteur_data = object_data.get(capteur_key, {})
                    ...
                    print(f"📈 Nombre d'enregistrements capteur de sol créés : {z}")
...
def device_status(request):
    ...
    last = {
        ...
        'Humectation': DataMBNUANCE.objects.only('Time_Stamp').order_by('-Time_Stamp').first(),
    ...
    devices = [
        ...
        {'name': 'Humectation',              'eui': '70b3d58f800005eb', 'type': 'LoRaWAN', 'last_seen': getattr(last['Humectation'],'Time_Stamp', None), 'threshold': 240},
...
# Importez votre modèle — ajustez le chemin si nécessaire
from application.models import DataMBNUANCE


# ──────────────────────────────────────────────────────────────
# DevEUI du module MBNUANCE (à adapter si besoin)
# ──────────────────────────────────────────────────────────────
MBNUANCE_DEV_EUI = "70b3d58f800005eb"   # ← remplacez par le vrai DevEUI


from typing import Union

def decode_mbnuance_payload(b: bytes) -> Union[dict, None]:
    """
    Reproduit fidèlement le codec JS ChirpStack :
      - bytes[12:16] → leaf_wetness_pct  (float32 Little-Endian)
      - bytes[16:20] → temperature_c     (float32 Little-Endian)
    Retourne None si le payload est trop court.
    """
    if len(b) < 20:
        return None

    wetness     = round(struct.unpack_from('<f', b, 12)[0], 2)
    temperature = round(struct.unpack_from('<f', b, 16)[0], 2)

    if   wetness <  5:  state = "dry"
    elif wetness < 30:  state = "slightly_wet"
    elif wetness < 70:  state = "wet"
    else:               state = "very_wet"

    return {
        "leaf_wetness_pct":   wetness,
        "leaf_wetness_state": state,
...
MBNUANCE_DEV_EUI = "70b3d58f800005eb"  # ⚠️ à remplacer
...
def mbnuance_uplink(request):
    ...
    if dev_eui != MBNUANCE_DEV_EUI:
        return JsonResponse({"status": "ignored"}, status=200)

    obj = data.get('object', {})

    leaf_wetness_pct   = obj.get('leaf_wetness_pct')
    leaf_wetness_state = obj.get('leaf_wetness_state')
    ...
    try:
        ...
        DataMBNUANCE.objects.all().delete()

        # ✅ garder seulement la dernière mesure
        record = DataMBNUANCE(
            devEui=dev_eui,
            leaf_wetness_pct=leaf_wetness_pct,
            leaf_wetness_state=leaf_wetness_state,
...
# 📊 DASHBOARD
# ─────────────────────────────────────────────────────────────
def mbnuance_dashboard(request):

    latest = DataMBNUANCE.objects.order_by('-Time_Stamp').first()
    ...
    return render(request, 'mbnuance_dashboard.html', context)


# ─────────────────────────────────────────────────────────────
# 📈 API CHART
# ─────────────────────────────────────────────────────────────
def mbnuance_chart_data(request):

    latest = DataMBNUANCE.objects.order_by('-Time_Stamp').first()

    if latest:
        return JsonResponse({
            'labels':      [latest.Time_Stamp.strftime('%d/%m %H:%M')],
            'wetness':     [latest.leaf_wetness_pct],
...
#from django.shortcuts import render
#from django.utils import timezone
from .models import ET0Journalier, CalibrationJournaliere

def et0_dashboard(request):
    ...
    et0_davis = ET0Journalier.objects.filter(
        station='davis', date__range=[date_debut, date_fin]
    ).order_by('date')

    et0_sensecap = ET0Journalier.objects.filter(
        station='sensecap', date__range=[date_debut, date_fin]
    ).order_by('date')

    calibrations = CalibrationJournaliere.objects.filter(
        date__range=[date_debut, date_fin]
    ).order_by('-date')[:10]

    derniere_davis    = et0_davis.last()
    derniere_sensecap = et0_sensecap.last()

    # Écart calculé en Python, pas en template
    ecart = None
    ecart_pct = None
    if derniere_davis and derniere_sensecap:
        ecart     = round(derniere_davis.ET0_mm - derniere_sensecap.ET0_mm, 2)
        ecart_pct = round(ecart / derniere_sensecap.ET0_mm * 100, 1) if derniere_sensecap.ET0_mm else None

    # Séries Chart.js : axe X = dates de Davis, SenseCap aligné par date
    labels        = [str(e.date) for e in et0_davis]
    serie_davis   = [round(e.ET0_mm, 2) for e in et0_davis]
    sc_dict       = {e.date: round(e.ET0_mm, 2) for e in et0_sensecap}
    serie_sensecap = [sc_dict.get(e.date) for e in et0_davis]

    context = {
        'derniere_davis':    derniere_davis,
        'derniere_sensecap': derniere_sensecap,
        'calibrations':      calibrations,
        'ecart':             ecart,
        'ecart_pct':         ecart_pct,
        'labels_json':       json.dumps(labels),
        'serie_davis_json':  json.dumps(serie_davis),
        'serie_sc_json':     json.dumps(serie_sensecap),
        'nb_jours':          nb_jours,
        'choix_jours':       [7, 14, 30, 90],
        'date_fin':          date_fin,
    }
    return render(request, 'irrigation/et0_dashboard.html', context)

@csrf_exempt
def receive_gssps_data(request):
    if request.method == 'POST':
        data = json.loads(request.body)
        object_data = data.get('object', {})

        # On extrait les données décodées par ton codec ChirpStack
        # (Assure-toi que les clés correspondent à celles du codec envoyé précédemment)
        inner_data = object_data.get('data', {})

        GSSPSProfile.objects.create(
            devEUI=data.get('devEUI'),
            battery_v=object_data.get('battery_v'),
            hum_0_40=inner_data.get('hum_0_40', 0),
            hum_40_60=inner_data.get('hum_40_60', 0),
            hum_60_100=inner_data.get('hum_60_100', 0),
            temp_0_40=inner_data.get('temp_0_40', 0),
            temp_40_60=inner_data.get('temp_40_60', 0),
            temp_60_100=inner_data.get('temp_60_100', 0)
        )
        return HttpResponse(status=200)

def profile_100cm_view(request):
    # Récupérer les données des dernières 24h ou derniers relevés
    data_points = GSSPSProfile.objects.all().order_by('time_stamp')[:50] # 50 relevés à 4h d'intervalle = ~8 jours

    context = {
        'labels': [d.time_stamp.strftime("%H:%M") for d in data_points],
        'h0_40': [d.hum_0_40 for d in data_points],
        'h40_60': [d.hum_40_60 for d in data_points],
        'h60_100': [d.hum_60_100 for d in data_points],
        'last_data': data_points.last()
    }
    return render(request, 'gssps_visualisation.html', context)
